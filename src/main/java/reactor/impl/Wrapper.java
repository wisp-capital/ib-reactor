...
import reactor.utils.PrettyPrinters;
...
public class Wrapper implements EWrapper {
    ...
    public Wrapper(
        ...
        Sinks.Many<IbLogRecord> logObserver
    ) {
        ...
        errorHandler = new TerminalErrorHandler(requests) {
            ...
            void onLog(IbLogRecord record) {
                logObserver.emitNext(record, Sinks.EmitFailureHandler.FAIL_FAST);
            }
    ...
    public void tickPrice(int tickerId, int field, double price, TickAttrib attribs) {
        ...
        IbTick result = cache.updateTick(tickerId,
            (tick) -> tick.setPriceValue(tickerId, field, BigDecimal.valueOf(price), attribs)
        );
    ...
    public void tickSize(int tickerId, int field, int value) {
        ...
        IbTick result =
            cache.updateTick(tickerId, (tick) -> tick.setIntValue(tickerId, field, value));
        publishNewTick(tickerId, result);
    }
    ...
    public void tickGeneric(int tickerId, int field, double value) {
        IbTick result = cache.updateTick(tickerId,
            (tick) -> tick.setGenericValue(tickerId, field, BigDecimal.valueOf(value))
        );
        publishNewTick(tickerId, result);
    }
    ...
    public void tickString(int tickerId, int field, String value) {
        IbTick result =
            cache.updateTick(tickerId, (tick) -> tick.setStringValue(tickerId, field, value));
        publishNewTick(tickerId, result);
    }
    ...
    public void orderStatus(
        ...
        if (cache.addNewStatus(twsStatus)) {
            ...
            if (twsStatus.isCanceled()) {
                requests.onNextAndComplete(RequestRepository.Type.REQ_ORDER_CANCEL,
                    orderId,
                    true,
                    false
                );
            ...
            if (twsStatus.isFilled()) {
                requests.onError(RequestRepository.Type.REQ_ORDER_CANCEL,
                    orderId,
                    new IbExceptions.OrderAlreadyFilledError(orderId),
                    false
                );
            }
        }
    ...
    public void openOrder(int orderId, Contract contract, Order order, OrderState state) {
        ...
        if (cache.addOrder(twsOrder)) {
            ...
            if (!state.status().isActive()) {
                requests.onError(RequestRepository.Type.REQ_ORDER_PLACE,
                    orderId,
            ...
            requests.onNextAndComplete(RequestRepository.Type.REQ_ORDER_PLACE,
                orderId,
                twsOrder,
                false
            );
    ...
    public void openOrderEnd() {
        requests.onNextAndComplete(RequestRepository.Type.REQ_ORDER_LIST,
            null,
            new ArrayList<>(cache.getOrders().values()),
            false
        );
    }
    ...
    public void updatePortfolio(
        ...
        IbPortfolio portfolio = new IbPortfolio(contract,
            positionObj,
            marketPriceObj,
            marketValueObj,
            averageCostObj,
    ...
    public void contractDetails(final int reqId, final ContractDetails contractDetails) {
        log.trace("contractDetails: reqId={}, details={}",
            reqId,
            PrettyPrinters.contractDetailsToString(contractDetails)
        );
    ...
    public void updateMktDepthL2(
        ...
        log.trace(
            "updateMktDepthL2: tickerId = {}, position = {}, marketMaker = {}, operation = {}, side = {}, " +
                "price = {}, size = {}",
            tickerId,
            position,
            marketMaker,
    ...
    public void managedAccounts(String accountsList) {
        ...
        managedAccounts = new HashSet<>(Splitter.on(",").splitToList(accountsList));
    }
    ...
    public void marketRule(int marketRuleId, PriceIncrement[] priceIncrements) {
        log.trace("marketRule: marketRuleId={}, priceIncrements={}",
            marketRuleId,
            PrettyPrinters.priceIncrementsToString(priceIncrements)
        );

        List<PriceIncrement> increments = Stream.of(priceIncrements).collect(Collectors.toList());
        requests.onNextAndComplete(RequestRepository.Type.REQ_MARKET_RULE,
            marketRuleId,
            increments,
            true
        );
    ...
    public void execDetails(final int reqId, final Contract contract, final Execution execution) {
        ...
        log.trace("execDetails: reqId={}, contract={}, execution={}",
            reqId,
            ibContract,
            ibExecution
        );
    ...
    public void commissionReport(final CommissionReport report) {
        ...
        cache.updateExecutionReport(commissionReport).ifPresent(execReport -> {
            log.info(
                "Order {} is executed with exec id '{}, total amount: {}, price: {}, commission: {} {}',  ",
                execReport.getExecution().getOrderId(),
                execReport.getExecution().getExecId(),
                execReport.getExecution().getCumQty(),
                execReport.getExecution().getPrice(),
                commissionReport.getCommission(),
                commissionReport.getCurrency()
            );
    ...
    public void marketDataType(final int reqId, final int marketDataType) {
        log.trace("marketDataType: NOT IMPLEMENTED - reqId={}, marketDataType={}",
            reqId,
            marketDataType
        );
    }
    ...
    public void position(String account, Contract contract, double pos, double avgCost) {
        ...
        IbPosition position =
            new IbPosition(account, contract, BigDecimal.valueOf(pos), BigDecimal.valueOf(avgCost));
    ...
    public void accountSummary(
        ...
        log.trace("accountSummary: reqId = {}, account = {}, {} = {} {}",
            reqId,
            account,
            tag,
            value,
    ...
    public void accountSummaryEnd(final int reqId) {
        requests.onNextAndComplete(RequestRepository.Type.REQ_ACCOUNT_SUMMARY,
            reqId,
            cache.popAccountsSummary(reqId),
            true
        );
    }
    ...
    public void positionMultiEnd(final int reqId) {
        requests.onNext(RequestRepository.Type.EVENT_POSITION_MULTI,
            reqId,
            IbPosition.COMPLETE,
            false
        );
    ...
    public void symbolSamples(int reqId, ContractDescription[] contractDescriptions) {
        ...
        Stream.of(contractDescriptions).map(IbContractDescription::new).forEach(desc -> {
    ...
    public void mktDepthExchanges(DepthMktDataDescription[] depthMktDataDescriptions) {
        List<IbDepthMktDataDescription> result = Lists.newArrayList(depthMktDataDescriptions)
            .stream()
            .map(IbDepthMktDataDescription::new)
            .collect(Collectors.toList());
        log.trace("mktDepthExchanges: {}", result);

        requests.onNextAndComplete(RequestRepository.Type.REQ_MARKET_DEPTH_EXCHANGES,
            null,
            result,
            true
        );
    ...
    public void tickReqParams(
        ...
        log.trace(
            "tickReqParams: NOT IMPLEMENTED - tickerId={}, minTick={}, bboExchange={}, snapshotPermissions={}",
            tickerId,
            minTick,
            bboExchange,
            snapshotPermissions
    ...
    public void pnl(int reqId, double dailyPnL, double unrealizedPnL, double realizedPnL) {
        ...
        IbPnl pnl = new IbPnl(null,
            BigDecimal.valueOf(dailyPnL),
    ...
    public void pnlSingle(
        int reqId, int pos, double dailyPnL, double unrealizedPnL, double realizedPnL, double value
    ) {
        log.trace(
    ...
    public void completedOrder(Contract contract, Order order, OrderState orderState) {
        log.trace("completedOrder: NOT IMPLEMENTED. order={}, state={}",
            order.orderId(),
    ...
    private void handleUpdateMktDepth(
        int tickerId,
        int position,
        String marketMaker,
        int operation,
        ...
        Contract contract =
            (Contract) requests.getUserData(RequestRepository.Type.EVENT_MARKET_DATA_LVL2,
                tickerId
            );
        Objects.requireNonNull(contract);

        IbMarketDepth orderBookDepth = new IbMarketDepth(contract,
            position,
            side,
        ...
        requests.onNext(RequestRepository.Type.EVENT_MARKET_DATA_LVL2,
            tickerId,
            orderBookDepth,
            true
        );
